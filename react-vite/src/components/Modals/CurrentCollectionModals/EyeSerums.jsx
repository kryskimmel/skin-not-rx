...
function EyeSerumsModal({collectionName}) {
    const dispatch = useDispatch();
    const user = useSelector(store => store.session.user);
    const eyeSerumsProducts = useSelector(store => store.product.byProductType[collectionName]);
    useEffect(() => { dispatch(productActions.getAllProducts())}, [dispatch]);

    const userProducts = eyeSerumsProducts.filter(product => product.user_id === user.id);
